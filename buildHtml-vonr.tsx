...
import processedvonr from "./processed-vonr.json";
...
let carriers = processedvonr as Record<string, { source: string, version: string, names: string[], country?: string, countryCode: string, data: CarrierPlist, blob: CarrierPlist}>;

let rcsStatus = (data: typeof carriers[string]) => (data.blob.SupportsVoNR || data.data.SupportsVoNR) ? (data.source.includes("DeveloperOS") ? 1 : 2) : 0;
...
const CarrierSupportTable = () => { 
    ...
    entries.sort(([aCountry,aCarriers],[bCountry,bCarriers]) => 
        (bCarriers?.filter(([id, data]) => (data.blob.SupportsVoNR || data.data.SupportsVoNR)).length ?? 0) -
        (aCarriers?.filter(([id, data]) => (data.blob.SupportsVoNR || data.data.SupportsVoNR)).length ?? 0) 
    );
...
let html = renderToString(<>
    <head>
        <title>Does my carrier support Voice over NR on iOS yet?</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta name="description" content="A list of carriers that support Voice over NR on iOS" />
...
writeFileSync("./html/vonr/index.html", html);
