...
import processedwasa from "./processed-watchsa.json";
...
let carriers = processedwasa as Record<string, { source: string, version: string, names: string[], country?: string, countryCode: string, data: CarrierPlist, blob: CarrierPlist}>;

let rcsStatus = (data: typeof carriers[string], id:string) => (lib.symbols.watchsa(Buffer.from(id, 'utf8'), id.length) || data.data.RemoteCardProvisioningSettings?.MinCompatibleWatchOSForStandaloneMode || data.blob.RemoteCardProvisioningSettings?.MinCompatibleWatchOSForStandaloneMode) ? (data.source.includes("DeveloperOS") ? 1 : 2) : 0;
...
const CarrierSupportTable = () => { 
    ...
    entries.sort(([aCountry,aCarriers],[bCountry,bCarriers]) => 
        (bCarriers?.filter(([id, data]) => (lib.symbols.watchsa(Buffer.from(id, 'utf8'), id.length) || data.data.RemoteCardProvisioningSettings?.MinCompatibleWatchOSForStandaloneMode || data.blob.RemoteCardProvisioningSettings?.MinCompatibleWatchOSForStandaloneMode)).length ?? 0) -
        (aCarriers?.filter(([id, data]) => (lib.symbols.watchsa(Buffer.from(id, 'utf8'), id.length) || data.data.RemoteCardProvisioningSettings?.MinCompatibleWatchOSForStandaloneMode || data.blob.RemoteCardProvisioningSettings?.MinCompatibleWatchOSForStandaloneMode)).length ?? 0) 
    );
...
let html = renderToString(<>
    <head>
        <title>Does my carrier support Cellular Standalone on watchOS yet?</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta name="description" content="A list of carriers that support Cellular Standalone on watchOS" />
    ...
    <body>
        <div class='container'>
            <header>
                <h1>Does my carrier support Cellular Standalone on watchOS yet?</h1>
                <p>
...
writeFileSync("./html/watchsa/index.html", html);
